 ...
 *
 */
@Service(AppLeaveFileUploadService.ID)
public class AppLeaveFileUploadServiceImpl implements AppLeaveFileUploadService{

	final static Logger logger = Logger.getLogger(AppLeaveFileUploadServiceImpl.class);
	...
	public JSONObject saveUploadInfo(HttpServletRequest reqParam) throws Exception {
		logger.info("AppLeaveFileUploadServiceImpl/saveUploadInfo");
		...
		for (int i = 0; i < jasonArray.size(); i++) {
			...
			systemFileModel.setTable_name(YJWYLeaveModel.META_ID);
