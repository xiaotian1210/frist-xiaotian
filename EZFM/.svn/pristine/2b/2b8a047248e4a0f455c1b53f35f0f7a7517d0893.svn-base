 ...
 *
 */
@Service(AppWorkTaskFileUploadService.ID)
public class AppWorkTaskFileUploadServiceImpl implements AppWorkTaskFileUploadService{

	final static Logger logger = Logger.getLogger(AppWorkTaskFileUploadServiceImpl.class);
	...
	public JSONObject saveUploadInfo(HttpServletRequest reqParam) throws Exception {
		logger.info("AppWorkTaskFileUploadServiceImpl/saveUploadInfo");
		...
		for (int i = 0; i < jasonArray.size(); i++) {
			...
			problemModel.setTable_name(YJWYWorkTaskDetailsRecordModel.META_ID);
			problemModel.setRecord_id(String.valueOf(mapModel.get("worktaskRecordId")));
