 ...
 *
 */
@Service(AppWorkTaskSignedFileUploadService.ID)
public class AppWorkTaskSignedFileUploadServiceImpl implements AppWorkTaskSignedFileUploadService{

	final static Logger logger = Logger.getLogger(AppWorkTaskSignedFileUploadServiceImpl.class);
	...
	public JSONObject saveUploadInfo(HttpServletRequest reqParam) throws Exception {
		logger.info("AppWorkTaskSignedFileUploadServiceImpl/saveUploadInfo");
		...
		for (int i = 0; i < jasonArray.size(); i++) {
			...
			problemModel.setTable_name(YJWYWorkTaskDetailsModel.META_ID);
			problemModel.setRecord_id(String.valueOf(mapModel.get("worktaskId")));
