...
public abstract class AndroidNotification extends UmengNotification {
	...
	// Keys can be set in the body level
/*	protected static final HashSet<String> PM_KEYS = new HashSet<String>(Arrays.asList(new String[]{
			"pm_module", "pm_function", "pm_ext"}));
*/
	...
	public boolean setPredefinedKeyValue(String key, Object value) throws Exception {
		...
		/*} else if(PM_KEYS.contains(key)){
			// This key should be in the body level
			JSONObject pmJson = null;
			JSONObject payloadJson = null;
			// 'body' is under 'payload', so build a payload if it doesn't exist
			if (rootJson.has("payload")) {
				payloadJson = rootJson.getJSONObject("payload");
			...
			// Get body JSONObject, generate one if not existed
			if (payloadJson.has("pm")) {
				pmJson = payloadJson.getJSONObject("pm");
			} else {
				pmJson = new JSONObject();
				payloadJson.put("pm", pmJson);
			}
			pmJson.put(key, value);*/
	...
	// Set extra key/value for Android notification
	public boolean setExtraField(Map<String, Object> map) throws Exception {
		JSONObject payloadJson = null;
		JSONObject extraJson = null;
		if (rootJson.has("payload")) {
			payloadJson = rootJson.getJSONObject("payload");
		...
		if (payloadJson.has("extra")) {
			...
			extraJson = new JSONObject();
			String pmFunctionVal = String.valueOf(map.get("pm_function"));
			String pmModule = String.valueOf(map.get("pm_module"));
			extraJson.put("pm_function", pmFunctionVal);
			extraJson.put("pm_module", pmModule);
			payloadJson.put("extra", extraJson);
		}
		return true;
	}
