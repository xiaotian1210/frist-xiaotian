...
import com.shareworx.ezfm.app.patrol.service.AppPatrolFileUploadService;
 ...
 */
@Controller
@RequestMapping("app/patrolFileUpload")
public class AppPatrolFileUploadAction {

	final static Logger logger = Logger.getLogger(AppPatrolFileUploadAction.class);
	@Autowired
	@Qualifier(AppPatrolFileUploadService.ID)
	private AppPatrolFileUploadService patrolFileUploadService;
	...
	public @ResponseBody JSONObject saveUploadInfo(HttpServletRequest reqParam) throws Exception{
		...
		try {
			JSONObject json = patrolFileUploadService.saveUploadInfo(reqParam);
