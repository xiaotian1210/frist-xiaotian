...
import com.shareworx.ezfm.app.worktask.service.AppWorkTaskFileUploadService;
 ...
 */
@Controller
@RequestMapping("app/workTaskFileUpload")
public class AppWorkTaskFileUploadAction {

	final static Logger logger = Logger.getLogger(AppWorkTaskFileUploadAction.class);
	@Autowired
	@Qualifier( AppWorkTaskFileUploadService.ID)
	private AppWorkTaskFileUploadService wtFileUploadService;
	...
	public @ResponseBody JSONObject saveUploadInfo(HttpServletRequest reqParam) throws Exception{
		...
		try {
			JSONObject json = wtFileUploadService.saveUploadInfo(reqParam);
